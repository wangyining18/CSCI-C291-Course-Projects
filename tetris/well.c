...
void draw_well(well_t *w) {
  ...
  srand(time(NULL));
  int type;
  obstruction_t * obstruct = malloc(sizeof(obstruction_t));
  type = rand()%7;
  memcpy(obstruct, &obstruction_types[type], sizeof(obstruction_t));
  obstruct->upper_left_y =w->upper_left_y + rand() % w->height  ;
  obstruct->upper_left_x =w->upper_left_x + rand() % w->width ;

  int x,y;
  for (x=0;x<4;x++) {
	  for (y=0;y<+4;y++) {
		  if (obstruct->piece[x][y]) {
			  mvprintw(obstruct->upper_left_y+y,obstruct->upper_left_x+x,"%c",obstruct->draw_char);
		  }
	  }
  }
